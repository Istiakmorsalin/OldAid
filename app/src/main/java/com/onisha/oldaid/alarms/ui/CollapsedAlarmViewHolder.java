...
import static com.onisha.oldaid.alarms.misc.DaysOfWeek.NUM_DAYS;

/**
 * Created by Onisha on 5/31/2016.
 */
public class CollapsedAlarmViewHolder extends BaseAlarmViewHolder {

    @Bind(com.onisha.oldaid.R.id.countdown) AlarmCountdown mCountdown;
    @Bind(com.onisha.oldaid.R.id.recurring_days) TextView mDays; // TODO: use `new DateFormatSymbols().getShortWeekdays()` to set texts

    public CollapsedAlarmViewHolder(ViewGroup parent, OnListItemInteractionListener<Alarm> listener,
                                    AlarmController alarmController) {
        super(parent, com.onisha.oldaid.R.layout.item_collapsed_alarm, listener, alarmController);
    }
    ...
    private void bindDays(Alarm alarm) {
        ...
        if (num == DaysOfWeek.NUM_DAYS) {
            text = getContext().getString(com.onisha.oldaid.R.string.every_day);
        } else if (num == 0) {
            text = "";
        } else {
